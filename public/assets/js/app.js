...
document.querySelectorAll(".modal-level").forEach(check => {
    check.addEventListener("click", e => {
        ...
        if(elem.type !== "materials"){
            ...
            document.querySelector("#modal-body-home .row:nth-child(2)").innerHTML="";
            document.querySelector("#modal-body-home .row:nth-child(2)").append(document.querySelector("#template-materials-" + level).content.cloneNode(true));
            const divModal =  document.querySelector("#modal-body-home .row:nth-child(2) div");
            //bold
            const bold = ("bold" in elem.temp) ? elem.temp.bold : elem.bold
            divModal.style.fontWeight = bold ? "bold" : "normal";
            //fontFamily
            const fontFamily = ("fontFamily" in elem.temp) ? elem.temp.fontFamily : elem.fontFamily
            divModal.style.fontFamily = fontFamily;
            //fontSize
            const fontSize = ("fontSize" in elem.temp) ? elem.temp.fontSize : elem.fontSize
            divModal.style.fontSize = fontSize + "px";
            //fontColor
            const fontColor =("fontColor" in elem.temp) ? elem.temp.fontColor : elem.fontColor
            divModal.style.color = fontColor;
            //backgroundColor
            const backgroundColor = ("bgColor" in elem.temp) ? elem.temp.bgColor : elem.bgColor
            divModal.style.backgroundColor = backgroundColor;
            //textToSpeechText
            const textToSpeechBool = ("textToSpeechBool" in elem.temp) ? elem.temp.textToSpeechBool : elem.textToSpeechText
            if(textToSpeechBool){
                ...
                divModal.querySelector(".material-buttons").append(btnmat);
            }
...
document.querySelector("#modal-speechToTextBool").addEventListener("input", e => {
    const elem = form.get(document.querySelector("#staticBackdrop").dataset.idElement);
    const divModal = document.querySelector("#modal-body-home .row:nth-child(2) div");
    if(e.currentTarget.checked) {
        if(divModal.querySelector(".btn-speech") === null) {
            if ((elem.type === "text" || elem.type === "textarea") &&
            elem.idElementForm !== "applicantName" &&
            elem.idElementForm !== "urgencyDegree" &&
            elem.idElementForm !== "location" &&
            elem.idElementForm !== "description") {
                        ...
                        speechtotext(divModal.querySelector(".div-input input"),btn);
                }
        }
        elem.speechToTextBool = true;
    }else {
        document.querySelector("#modal-body-home .row:nth-child(2) .btn-speech")?.remove();
        elem.speechToTextBool = false;
    }
});
...
// italic
document.querySelector("#global-speechToTextBool").addEventListener("input", e => {
    form.forEach(element => {
        if ((element.type === "text" || element.type === "textarea") &&
            element.idElementForm !== "applicantName" &&
            element.idElementForm !== "urgencyDegree" &&
            element.idElementForm !== "location" &&
            element.idElementForm !== "description") {
        element.speechToTextBool = e.currentTarget.checked;
        element.updateDOM();
            }
    });
});
...
document.addEventListener("keyup", e => {
    ...
    switch(e.key) {
        ...
        case "i":
        case "I":
            const checkboxSpeechToTextBool = document.querySelector("#modal-checkboxSpeechToTextBool");
            checkboxSpeechToTextBool.checked = !checkboxSpeechToTextBool.checked;
            checkboxSpeechToTextBool.dispatchEvent(new Event("input"));
            break;
        case "b":
        case "B":
