...
<div class="row mx-0">
        <div class="d-flex justify-content-between align-items-center px-0">
            <div>
                <?php
                    if(\app\class\Breadcrumb::contains('infoSession')){
                        \app\class\Breadcrumb::removeAfter('infoSession');
                        \app\class\Breadcrumb::add('infoSession', $form->session->wording, 'bi-folder', "/sessions-".$form->session->idSession);
                        \app\class\Breadcrumb::add('infoForm', 'Fiche de '.$student->firstName . " " . $student->lastName, 'bi-file-earmark',"/etudiants" . "-" . $student->idUser . "/fiche-".$form->form->numero);
                    }else{
                        \app\class\Breadcrumb::removeAfter('infoForm');
                        \app\class\Breadcrumb::add('infoForm', 'Fiche n° '.$form->form->numero, 'bi-file-earmark',"/etudiants" . "-" . $student->idUser . "/fiche-".$form->form->numero);
                    }
    ...
    <?php 
    if(in_array($_SESSION["role"], ["educator-admin", "super-admin"])) { ?>
    <div class="row  mx-0 d-flex mb-3">
        <div class="d-flex px-0">
        <?php
      if(!empty($form) && !empty($form->form) && !$form->form->finish) { ?>
        <div class="px-0 me-3">
            <form action="<?= $_SERVER["REQUEST_URI"] ?>" method="POST">
                <button type="submit" class="btn btn-primary btn-finish-form"> <i class="bi bi-x-lg me-2"></i>Terminer</button>
    ...
    <?php }elseif(!empty($form) && !empty($form->form) && $form->form->finish){ ?>
        <div class="px-0 me-3">
            <form action="<?= $_SERVER["REQUEST_URI"] ?>" method="POST">
                <button type="submit" class="btn btn-primary btn-open-form "> <i class="bi bi-key"></i> Ouvrir</button>
        ...
        <?php }?>
            <div class="px-0">
                <form action="<?= $_SERVER["REQUEST_URI"] ?>" method="POST">
                    <button type="submit" class="btn btn-primary btn-remove-form"><i class="me-2 bi bi-trash"></i>Supprimer</button>
...
<div class="modal fade" id="ModalComs" tabindex="-1" aria-labelledby="ModalComs" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="col-12 mt-2">
                        ...
                        <input class=" form-control" id="formNote" name="note" pattern="[0-9]|1[0-9]|20">
                    </div>
                    <div class="form-check form-switch mt-2">
                        <input class="form-check-input" type="checkbox" role="switch" id="admin" name="admin">
                        <label class="form-check-label" for="flexSwitchCheckDefault">Visible uniquement par l'équipe pédagogique</label>
                    </div>
            </div>
        ...
        <form action="<?= $_SERVER["REQUEST_URI"] ?>" method="POST">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-check form-switch mt-2">
                        <input class="form-check-input" type="checkbox" role="switch" id="admin" name="admin" value="1">
                        <label class="form-check-label" for="flexSwitchCheckDefault">Visible uniquement par l'équipe pédagogique</label>
                    </div>
                </div>
                <div class="modal-footer">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-3">
                            ...
                            <input id="inputImgForm" type="file" class="d-none" name="picture">
                        </div>
                        <div class=" col-6">
                            <label for="pictureTitle" class="form-label pe-none">Choisissez un titre</label>
                            <input class=" form-control" id="pictureTitle" name="title">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-cancel btn-danger me-2" data-bs-dismiss="modal">
    ...
    <template id="takePhotoContainer">
        <div class="container">
            <div class="row">
                <video id="videoCam" class="bg-dark vw-100 vh-100"></video>
            </div>
            <div class="row position-absolute bottom-0 start-0 end-0 bg-dark" style="height: 100px;">
                <div class="col d-flex justify-content-center align-items-center">
                    <button class="btn btn-secondary w-100 h-100 z-2"><i class="bi bi-arrow-repeat"></i></button>
                </div>
                <div class="col d-flex justify-content-center align-items-center">
    ...
    <template id="confirmPhotoContainer">
        <div class="container">
            ...
            <form id="photoForm" enctype="multipart/form-data" method="POST" action="<?=$_SERVER['REQUEST_URI']?>">
                ...
                <div class="row position-absolute bottom-0 start-0 end-0 z-2" style="height: 100px;">
                    <div class="col-12">
                        <input id="inputTitlePicture" name="title">
                    </div>
                    <div class="col d-flex justify-content-center align-items-center">
